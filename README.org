...
while sexp from =:custom-after-load= argument are evaluated after the package is loaded.
=:custom-after-load= is used when you want to use initial value of customized variable
or function defined in the package.
...
#+begin_src emacs-lisp :tangle yes
  (mic feature-name
    ...
    ((variable3 . (+ variable3 1))
     ;; You can use function defined in the package (for this example `feature-name')
...
This exists in order to define =COMMAND= in the package with =KEYS= to =KEYMAP= defined in =FEATURE=.
...
#+begin_src emacs-lisp :tangle yes
  (mic feature-name
    ...
    ;; When `other-package-mode-map' is defined in `other-package', which is not `feature-name',
    ;; use `:define-key-with-feature'.
    :define-key-with-feature
    ((other-package
      (other-package-mode-map
  ...
  (mic-core feature-name
    ...
    ((define-key global-map (kbd "M-l") #'feature-name-command1)
     ...
     (with-eval-after-load 'other-package
       (define-key other-package-mode-map (kbd "M-q") #'feature-name-command5)))
    :eval-after-load
