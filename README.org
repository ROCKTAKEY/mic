...
#+begin_src emacs-lisp :tangle yes
  (mic dired-filter
    ;; `dired-filter-map' should be available when `dired' is required,
    ;; but there is no function to call when it is required.
    :define-key-with-feature
    ((dired
      (dired-mode-map
       ("/" . dired-filter-map))))
    ;; So just require `dired-filter' after `dired'
    :require-after
    ((dired . (dired-filter))))

  ;; Expand to:
  (mic-core dired-filter
    :eval-installation nil
    :eval
    ((with-eval-after-load 'dired
